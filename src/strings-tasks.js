...
function formatTime(minutes, seconds) {
  const first = minutes.toString().padStart(2, '0');
...
function countVowels(str) {
  const withoutVowels = str.replaceAll(/[aeiouyAEIOUY]+/gm, '');
...
function isPalindrome(str) {
  const strWSpace = str.replaceAll(/[\s!?,]+/gm, '').toLowerCase();
  return strWSpace === strWSpace.split('').reverse().join('');
}
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (el) => {
    const base = el <= 'Z' ? 65 : 97;
    return String.fromCharCode(base + ((el.charCodeAt(0) - base + 13) % 26));
  });
}
